...
- User's Preferred Session Duration: ${sessionDuration} minutes (use this as BASELINE)
- User's Preferred Break Duration: ${breakDuration} minutes (use this as BASELINE)
- In flexible mode: Can adjust session duration ±15 mins based on topic priority (range: ${Math.max(25, sessionDuration - 15)}-${Math.min(90, sessionDuration + 15)} mins)
...
1. **FEWER TOPICS, MORE DEPTH**: Schedule 3-5 topics maximum for a productive day. Each topic needs focused work based on user's preferred duration (${sessionDuration} mins baseline).
2. **Prioritize ruthlessly**: First topics in the list get prime time slots and best focus hours.
3. **Realistic breaks**: Add ${breakDuration}-minute breaks strategically (after 2-3 hours of work, or between very different subjects). NOT after every single session.
4. **Duration based on user preference + difficulty** (using ${sessionDuration} min as baseline):
   - Low confidence (1-4): ${Math.min(90, sessionDuration + 15)} minutes (baseline + 15)
   - Medium confidence (5-7): ${sessionDuration} minutes (user's exact preference)  
   - High confidence (8-10): ${Math.max(25, sessionDuration - 15)} minutes (baseline - 15)
