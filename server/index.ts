...
app.post('/api/send-verification-code', async (req: Request, res: Response) => {
  try {
    ...
    const fromAddress = process.env.RESEND_FROM_EMAIL || 'Vistara <noreply@vistara-ai.app>';
    ...
    const { data, error: emailError } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h1 style="color: #6366f1; text-align: center;">Vistara</h1>
  ...
  "difficult_topics": [
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: { 
        ...
        "HTTP-Referer": process.env.REPLIT_DEV_DOMAIN || "https://vistara.app"
      },
      body: JSON.stringify({
