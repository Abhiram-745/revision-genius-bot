...
import { Users, Clock, TrendingUp, Flame, BookOpen } from "lucide-react";
...
const StudyOverview = ({ userId }: StudyOverviewProps) => {
    ...
    try {
      ...
      if (streakData) {
        ...
        for (const s of streakData) {
          if (s.date === todayStr || s.date === yesterdayStr || currentStreak > 0) {
            currentStreak++;
          } else {
            break;
          }
      ...
      console.error("Error loading study overview:", error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-primary/10 to-accent/5 border-primary/20">
          <CardContent className="p-4 flex flex-col items-center text-center">
            ...
            <span className="text-2xl font-bold text-foreground">{stats.friendsCount}</span>
        ...
        <CardContent>
          {topicDistribution.length > 0 ? (
            ...
            <div className="h-64 flex items-center justify-center text-muted-foreground">
              <p>No study data yet. Complete some sessions to see your distribution!</p>
            </div>
          )}
        </CardContent>
      </Card>
