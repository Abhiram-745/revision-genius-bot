...
import { Zap, Shield, Clock, Brain, Heart, Sparkles } from "lucide-react";
...
const WhyStudentsLoveUs = () => {
  ...
  return (
    <section className="py-12 md:py-24 px-4 md:px-6 bg-gradient-to-b from-background via-muted/30 to-background relative overflow-hidden">
      ...
      {/* Laughing Owl Mascot - smaller on mobile */}
      <motion.img
        src={laughingOwl}
        alt="Happy owl mascot"
        className={`absolute bottom-4 left-2 md:bottom-8 md:left-8 ${
          isMobile ? 'w-20 h-20' : 'w-36 h-36 md:w-44 md:h-44 lg:w-52 lg:h-52'
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          <motion.span
            ...
            viewport={{ once: true }}
            className="inline-block px-3 py-1 md:px-4 md:py-1.5 rounded-full bg-primary/10 text-primary text-xs md:text-sm font-medium mb-3 md:mb-4"
          >
            ...
            >
              <Card className="h-full bg-card/80 backdrop-blur-sm border-2 border-transparent hover:border-primary/20 transition-all duration-300 overflow-hidden">
                <CardContent className="p-3 md:p-6 relative">
                  <motion.div
                    className="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center mb-2 md:mb-4 relative z-10"
                  ...
                  <h3 className="text-sm md:text-lg font-display font-bold mb-1 md:mb-2 relative z-10">
                    {feature.title}
                  </h3>
                  <p className="text-xs md:text-base text-muted-foreground relative z-10 line-clamp-2 md:line-clamp-none">
                    {feature.description}
                  </p>
                </CardContent>
              </Card>
