...
export const useUserRole = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Admin users are always premium (case-insensitive check)
...
export const checkCanCreateTimetable = async (): Promise<boolean> => {
  ...
  // Admins can always create (case-insensitive)
  if (ADMIN_EMAILS.includes((user.email || '').toLowerCase())) {
    return true;
  }
...
export const checkCanRegenerateTimetable = async (): Promise<boolean> => {
  ...
  // Admins can always regenerate (case-insensitive)
  if (ADMIN_EMAILS.includes((user.email || '').toLowerCase())) {
    return true;
  }
...
export const checkCanUseDailyInsights = async (): Promise<boolean> => {
  ...
  // Admins can always use (case-insensitive)
  if (ADMIN_EMAILS.includes((user.email || '').toLowerCase())) {
    return true;
  }
...
export const checkCanGenerateAIInsights = async (): Promise<boolean> => {
  ...
  // Admins can always generate (case-insensitive)
  if (ADMIN_EMAILS.includes((user.email || '').toLowerCase())) {
    return true;
  }
