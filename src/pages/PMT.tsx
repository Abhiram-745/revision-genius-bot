...
import { ArrowLeft, GraduationCap, Target, Clock, TrendingUp, BookOpen, Play, Star, Radio, ExternalLink } from "lucide-react";
import Header from "@/components/Header";
import PageTransition from "@/components/PageTransition";
import { supabase } from "@/integrations/supabase/client";
import { useAuth } from "@/context/AuthContext";
import PMTPracticeSession from "@/components/PMTPracticeSession";
...
const PMT = () => {
  ...
  useEffect(() => {
    const fetchTopicsAndStats = async () => {
      ...
      try {
        ...
        const scheduledTopics = todaySchedule.map((s: any) => s.topic?.toLowerCase());
        ...
        const topics: TopicWithStats[] = selectedTimetable.topics.map((topic: any) => {
          ...
          return {
            ...
            isRecommended: scheduledTopics.includes(topic.name?.toLowerCase()),
          };
        });
  ...
  const handleStartPractice = (subject: string, topic: string) => {
    const selectedTimetable = timetables.find(t => t.id === selectedTimetableId);
    const subjectData = selectedTimetable?.subjects.find((s: any) => s.name === subject);
    setPracticeSession({ subject, topic, examBoard: subjectData?.exam_board });
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-blue-500/5">
        ...
        <main className="container mx-auto px-4 py-6 space-y-6">
          ...
          <motion.div
            ...
            className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-500/20 via-blue-500/10 to-indigo-500/10 border border-blue-500/30 p-6 md:p-8"
          >
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-6">
              ...
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-3 flex-wrap">
                  <h1 className="text-2xl md:text-3xl font-bold text-foreground">Physics & Maths Tutor</h1>
                  <Badge className="bg-blue-500/20 text-blue-600 border-blue-500/30">External</Badge>
                </div>
                <p className="text-muted-foreground text-sm md:text-base max-w-2xl">
                  Access comprehensive revision resources including notes, worksheets, and past papers.
                </p>
              </div>
            </div>
          </motion.div>
            ...
            <Card className="bg-card/50">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">PMT Sessions</p>
                </div>
              </CardContent>
            </Card>
            <Card className="bg-card/50">
          ...
          {/* Recommended Topic */}
          {recommendedTopic && (
            <Card className="border-2 border-blue-500/30 bg-gradient-to-r from-blue-500/10 to-indigo-500/5">
              <CardHeader className="pb-2">
                <div className="flex items-center gap-2">
                  <Star className="w-5 h-5 text-blue-500" />
                  <CardTitle className="text-lg">Recommended Topic</CardTitle>
                </div>
              </CardHeader>
              <CardContent>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{recommendedTopic.name}</p>
                    <p className="text-sm text-muted-foreground">{recommendedTopic.subjectName}</p>
                  </div>
                  <Button
